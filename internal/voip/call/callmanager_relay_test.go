...
func TestBuildRelayConfigs(t *testing.T) {
	endpoints := []core.RelayEndpoint{
		...
		{IP: "5.5.5.5", Port: 3481, Protocol: 0, Key: "k5", RawToken: []byte{5}, RelayName: ""},
	}
	...
	if got[1].IP != "5.5.5.5" || got[1].Name != "5.5.5.5" || got[1].Port != 3481 {
		t.Errorf("expected name fallback to IP and announced port for second config: %+v", got[1])
	}
}
